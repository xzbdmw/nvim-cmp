...
ghost_text_view.text_gen = function(self, line, cursor_col, entry)
  ...
  local function line_chars_before(pos)
    ...
    local l = vim.api.nvim_buf_get_lines(0, pos[1] - 1, pos[1] + 1, false)
    return string.sub(l[1], 1, pos[2])
  end
  ...
  local snip = false
  local luasnip = nil
  if completion_item ~= nil and completion_item.kind == 15 and completion_item.data ~= nil then
    snip = true
    ...
    if vim.bo.expandtab then
      luasnip:expand_tabs(tab_width(), #indentstring)
    end
    luasnip:indent(indentstring)
    local text = luasnip:get_static_text()
    if snip then
      text[1] = trim_text(text[1])
    end
